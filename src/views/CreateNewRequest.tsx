...
const CreateNewRequest = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:4500/api/employees/all-employees').then((res) => {
      setEmployeeList(res.data)
    })
    axios.get(`http://localhost:4500/api/employees/${user?._id}`).then((res) => {
  ...
  const addReviewer = (reviewer: Employee) => {
    if (reviewerList!.length < 5) {
      ...
      toast.error('Maximum number of reviewers reached')
    }
  }

  // remove reviewer
  const removeReviewer = (reviewer: Employee) => {
    let newReviewerArray = reviewerList!.filter((user) => user._id !== reviewer._id)
    setReviewerList(newReviewerArray)
  }
    ...
    if (filterUser === '') {
      ...
      return employeeList!
        ...
        .filter((employee: Employee) => {
          return employee.displayName.toLowerCase().includes(filterUser.toLowerCase())
        })
        .map((employee) => {
  ...
  const renderUserCard = (prop: Employee) => {
    return (
      <Card sx={{ width: '300px', height: '70px', margin: '5px' }}>
        <CardHeader
          avatar={<Avatar sx={{ bgcolor: 'green' }}>{prop.displayName.slice(0, 1)}</Avatar>}
  ...
  const renderFormAction = () => {
    if (reviewerList?.length === 5 && dueDate) {
      return (
        <>
          <Typography>
            After you create this request, it needs to be confirmed by HR before everyone can give
            feedbacks
          </Typography>
      ...
      return (
        <>
          <Typography>Please choose a due date and 5 reviewers for this request!</Typography>
  ...
  const createRequest = () => {
    ...
    axios
      ...
      .then((res) => toast.success('Created new notification successfully'))
    ...
    <Stack sx={{ textAlign: 'left' }}>
      {isLoading === false ? (
        ...
        >
          ...
          <Stack flexGrow={4}>
            <Typography variant="h4">Reviewers: ({reviewerList?.length}/5)</Typography>
            <Stack
              direction={'row'}
              flexWrap={'wrap'}
              alignItems={'center'}
          ...
          <Modal open={openModal} onClose={handleModalClose} keepMounted sx={modalStyle}>
            <>
              ...
              >
                ...
                <Button variant="contained" color="success" onClick={createRequest}>
